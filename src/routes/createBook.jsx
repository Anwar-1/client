...
function createBook() {
  ...
  const createBook = async (e) => {
    ...
    try {
      ...
      //const response = await fetch("http://localhost:8000/api/books", {
        const response = await fetch("https://server-fzg6.onrender.com/api/books", {
        method: "POST",
        body: formData,
      });
  ...
  const onImageChange = (e) => {
    ...
    // setImage(e.target.files[0]); // هيك اعطيتيه اسم اي صورة بس ما حكيت الو وين موجودة 
     console.log(e.target.files[0])
     console.log(URL.createObjectURL(e.target.files[0])) // الها علاقة كيف صورة تنعرض على الصفحة 
     setThumbnail(e.target.files[0]); // في باك اند بس ببعت اسم الصورة 
    }
  }
